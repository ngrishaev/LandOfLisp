...
(defun draw-tile-svg (x y pos hex xx yy col chosen-tile)
  ...
  (loop for z below (second hex)
        do (draw-die-svg (+ xx (* *dice-scale* 0.3 (if (oddp (+ x y z)) -0.3 0.3)))
        ...
        (tag center ()
          ...
          (let ((chosen (assoc 'chosen params)))
            (when (or (not *cur-game-tree*) (not chosen))
              (setf chosen nil)
              (web-initialize))
            (cond ((lazy-null (caddr *cur-game-tree*)) (web-announce-winner (cadr *cur-game-tree*)))
                  ((zerop (car *cur-game-tree*)) (web-handle-human (when chosen (read-from-string (cdr chosen)))))
...
(defun web-handle-human (pos)
  (cond
    ...
    ((eq pos *from-tile*) (setf *from-tile* pos)
                          (princ "Now choose a destination:"))
